...
mysql_query('set names utf8');
	if(isset($_POST['btn']))//数据修改
	{
		...
		//拼接sql语句
		$sql="update bikerent set name='$username',phonenum='$phonenum',sno='$sno',bikenum='$bikenum',notes='$notes',time='$time' where id=$id"; 
		mysql_query($sql);
	}
//取出id对应的数据
$sql="select * from bikerent where id=$id";
$rs=mysql_query($sql);
		...
		<div class="container">
			<div class="row">
                ...
                <form id="form1" class="form-horizontal" name="form1" method="post" action="" onSubmit="return check()"> 
						<span class="heading">租车数据修改(管理员)</span>
					
                    
                 <div class="form-group"><input type="text" class="form-control" name="username" id="username" placeholder="姓名" value="<?php echo  $rows['name'] ?>"><i class="fa fa-user"></i> </div>
...
$url="http://www.xrone.cn/ahebike/ahebikerent.php";  
