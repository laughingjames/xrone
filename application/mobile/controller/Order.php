...
class Order extends MobileBase
{
	...
	function index(){		
		$this->assign('status',config('default_order_status_id'));
		$this->assign('top_title','我的订单/My Order');
		$this->assign('SEO',['title'=>'我的订单-My Order'.config('SITE_TITLE')]);
		if(in_wechat()){		
	...
	function ajax_order_list(){
		...
		if($status==0)
		    $status=config('default_order_status_id');
		//开始数字,数据量
		$limit = (5 * $page) . ",5";
	...
	function order_info(){
		...
		$this->assign('SEO',['title'=>'订单详情-Order Details'.config('SITE_TITLE')]);
		$this->assign('top_title','订单详情/Order Details');
		return $this->fetch();
	}
    ...
    function order_confirm(){
        osc_order()->order_confirm((int)input('param.order_id'),UID);
        storage_user_action(UID,user('nickname'),config('FRONTEND_USER'),'确认收货');
        return 1;
    }
