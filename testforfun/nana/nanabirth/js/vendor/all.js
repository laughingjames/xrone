		...
		this.each(function() {
			...
			*/
			...
			OLD BROWSER NOTES:
				...
				div = cycle
					div = slide
						index-img = full screen size image
					div = slide
						index-img = full screen size image
					etc.
			*/
			...
			var Old = {
				setup: function(){
					...
					if($.Slides.length > 0){
						...
						for(var j in $.Slides) {
							...
							if($('.mc-image-n' + j).children('index-img').length == 0){
							}else{
								// Add first image to loader to get that started
								$('div.mc-loader').append( $('.mc-image-n' + j).children('index-img').first().clone().addClass('not-loaded') );
							}
						}
				...
				preload: function(){
					...
					var t = setInterval(function() {
						$('.mc-loader').children('index-img').each(function(i){
							...
							if($img.hasClass('not-loaded')){
								if( $img.height() > 0 ){
									...
									// Set the dimensions
									var $img1 = $('div.mc-image-n' + i).children('index-img').first();
			...
			var Utils = {
				...
				construct_slide_object: function(){
					...
					$self.children().each(function(i){
						var $img = $(this).is('img') ? $(this).clone() : $(this).find('index-img').first().clone();
						...
						if($(this).find('index-img').length > 0){
							if($.BrowserTests.cssBackgroundSize){
								$(this).find('index-img').first().remove();
							}
