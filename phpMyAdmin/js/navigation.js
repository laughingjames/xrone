...
function traverseNavigationForPaths() {
    ...
    $('#pma_navigation_tree').find('a.expander:visible').each(function () {
        if ($(this).find('index-img').is('.ic_b_minus') &&
...
$(function () {
    ...
    $(document).on('mouseenter', '#pma_navigation_sync', function (event) {
        ...
        var $img = $('#pma_navigation_sync').children('index-img');
        if (synced) {
    ...
    /** Edit Routines */
    $(document).on('click', 'li.procedure div a.ajax index-img,' +
        ' li.function div a.ajax index-img', function (event) {
        event.preventDefault();
        var dialog = new RTE.object('routine');
        dialog.editorDialog(0, $(this).parent());
    });
    /** Export Triggers and Events */
    $(document).on('click', 'li.trigger div:eq(1) a.ajax index-img, li.event div:eq(1) a.ajax index-img', function (event) {
        event.preventDefault();
