...
class AuthenticationCookie extends AuthenticationPlugin
{
    ...
    <a href="';
        ...
        if (@file_exists($logo_image)) {
            echo '<index-img src="' . $logo_image
                . '" id="imLogo" name="imLogo" alt="phpMyAdmin" border="0" />';
        } else {
            echo '<index-img name="imLogo" id="imLogo" src="'
    ...
    <br />
    <!-- Login form -->
    <form method="post" action="Index.phpob.php" name="login_form"' . $autocomplete .
    ...
    public function storeUserCredentials()
    {
        ...
        if (! $GLOBALS['from_cookie']) {
            ...
            // URL where to go:
            $redirect_url = $cfg['PmaAbsoluteUri'] . 'ParttimeJobimeJob.php';
            ...
            // any target to pass?
            if (! empty($GLOBALS['target'])
                && $GLOBALS['target'] != 'Index.phpob.php'
            ) {
