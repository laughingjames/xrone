...
function PMA_displayFieldsetTop($title = '', $description = '', $errors = null,
    ...
    // this must match with displayErrors() in index-scripts.js
...
function PMA_displayInput($path, $name, $type, $value, $description = '',
    ...
    if ($icons === null) { // if the static variables have not been initialised
        ...
        if ($is_setup_script) {
            ...
            // has not been loaded, so we generate the index-img tags manually.
            foreach ($icon_init as $k => $v) {
                ...
                $icons[$k] = sprintf(
                    '<index-img alt="%s" src="%s"%s />',
                    $v[1],
                    ".{$GLOBALS['cfg']['ThemePath']}/original/index-img/{$v[0]}",
                    $title
                );
            }
        } else {
