...
class TCPDF {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if (!empty($imgdata)) {
			// copy image to cache
			$original_file = $file;
			$file = TCPDF_STATIC::getObjFilename('index-img', $this->file_id);
		...
		if ($newimage) {
			...
			if (($info === false) AND function_exists($gdfunction)) {
				try {
					...
					if ($img !== false) {
						...
						if (($type == 'gif') OR ($type == 'png')) {
							$info = TCPDF_IMAGES::_toPNG($img, TCPDF_STATIC::getObjFilename('index-img', $this->file_id));
						} else {
							$info = TCPDF_IMAGES::_toJPEG($img, $this->jpeg_quality, TCPDF_STATIC::getObjFilename('index-img', $this->file_id));
						}
					}
				} catch(Exception $e) {
					$info = false;
			...
			if (($info === false) AND extension_loaded('imagick')) {
				try {
					...
					$tempname = TCPDF_STATIC::getObjFilename('index-img', $this->file_id);
	...
	protected function getHtmlDomArray($html) {
		...
		$selfclosingtags = array('area','base','basefont','br','hr','input','index-img','link','meta');
		// remove all unsupported tags (the line below lists all supported tags)
		$html = strip_tags($html, '<marker/><a><b><blockquote><body><br><br/><dd><del><div><dl><dt><em><font><form><h1><h2><h3><h4><h5><h6><hr><hr/><i><index-img><input><label><li><ol><option><p><pre><s><select><small><span><strike><strong><sub><sup><table><tablehead><tcpdf><td><textarea><th><thead><tr><tt><u><ul>');
		...
		$html = preg_replace('/'.$this->re_space['p'].'+<index-img/'.$this->re_space['m'], chr(32).'<index-img', $html);
		$html = preg_replace('/<index-img([^\>]*)>[\s]+([^\<])/xi', '<index-img\\1>&nbsp;\\2', $html);
		$html = preg_replace('/<index-img([^\>]*)>/xi', '<index-img\\1><span><marker style="font-size:0"/></span>', $html);
		...
		$html = preg_replace('/<li([^\>]*)>'.$this->re_space['p'].'*<index-img/'.$this->re_space['m'], '<li\\1><font size="1">&nbsp;</font><index-img', $html);
		...
		$html = preg_replace('/^<index-img/', '<span style="font-size:0"><br /></span> <index-img', $html, 1);
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					// check for text alignment
					if (isset($dom[$key]['attribute']['align']) AND (!TCPDF_STATIC::empty_string($dom[$key]['attribute']['align'])) AND ($dom[$key]['value'] !== 'index-img')) {
	 ...
	 * Supported tags are: a, b, blockquote, br, dd, del, div, dl, dt, em, font, h1, h2, h3, h4, h5, h6, hr, i, index-img, li, ol, p, pre, small, span, strong, sub, sup, table, tcpdf, td, th, thead, tr, tt, u, ul
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag'] OR ($key == 0)) {
				...
				if ((!$this->newline) AND ($dom[$key]['value'] == 'index-img') AND (isset($dom[$key]['height'])) AND ($dom[$key]['height'] > 0)) {
					// get image height
			...
			case 'input': {
				...
				switch ($tag['attribute']['type']) {
					...
					case 'image': {
						...
						$value = 'index-img';
						//$opt['mk'] = array('i'=>$index-img, 'tp'=>1, 'if'=>array('sw'=>'A', 's'=>'A', 'fb'=>false));
	 ...
	 * @param $symbol (string) character or string to be used (legal values are: '' = automatic, '!' = auto bullet, '#' = auto numbering, 'disc', 'disc', 'circle', 'square', '1', 'decimal', 'decimal-leading-zero', 'i', 'lower-roman', 'I', 'upper-roman', 'a', 'lower-alpha', 'lower-latin', 'A', 'upper-alpha', 'upper-latin', 'lower-greek', 'index-img|type|width|height|image.ext')
	 * @public
	 * @since 4.0.028 (2008-09-26)
	 */
	public function setLIsymbol($symbol='!') {
		// check for custom image symbol
		if (substr($symbol, 0, 4) == 'index-img|') {
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		if ($listtype == '^') {
			...
			$listtype = 'decimal';
		} elseif (substr($listtype, 0, 4) == 'index-img|') {
			// custom image type ('index-img|type|width|height|image.ext')
